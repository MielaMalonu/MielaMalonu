...
          }
            // THIS LINE IS REMOVED: No longer extract 'priezastis' from the DOM if it's not needed.
...
        _0x3753bf += 0x3;
      }
        // THIS LINE IS REMOVED: No longer compare 'PRIEŽASTIS' for matching if not needed for deletion criteria
...
    }
    if (_0xa9739.PCCHECK) {
      _0x5ded9d += "&PCCHECK=eq." + encodeURIComponent(_0xxa9739.PCCHECK);
    }
    if (_0xa9739.ISP) {
      _0x5ded9d += "&ISP=eq." + encodeURIComponent(_0xa9739.ISP);
    }
    // THIS LINE IS REMOVED: No longer include 'PRIEŽASTIS' in the delete URL if not needed for the filter.
...
    var _0x33db13 = formatDate(_0x170c1c.DATA);
    // Removed the variable that checks for "rejected" status, as it's no longer used for rendering the rejection reason.
...
    // The current query is: "ID,METAI,PL,KL,PCCHECK,ISP,STATUS,DATA,NICK,REASON"
    // If you want to remove PRIEŽASTIS from fetching, ensure it's not in the select string.
